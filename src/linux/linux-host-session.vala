namespace Frida {
	...
	public sealed class LinuxHostSession : LocalHostSession {
		...
		private string agent_name_prefix;
	...
	construct {
		...
		agent_name_prefix = GLib.Uuid.string_random ();
		agent = new AgentDescriptor (PathTemplate (agent_name_prefix + "-<arch>.so"),
			new Bytes.static (blob32.data),
			new Bytes.static (blob64.data),
			new AgentResource[] {
				new AgentResource (agent_name_prefix + "-arm.so", new Bytes.static (emulated_arm.data), tempdir),
				new AgentResource (agent_name_prefix + "-arm64.so", new Bytes.static (emulated_arm64.data), tempdir),
			},
			AgentMode.INSTANCED,
			tempdir);
#endif
	...
	protected override string? get_emulated_agent_path (uint pid) throws Error {
		...
		switch (cpu_type_from_pid (pid)) {
			...
			case Gum.CpuType.AMD64:
				name = agent_name_prefix + "-arm64.so";
				break;
			default:
